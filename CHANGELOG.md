...
- `openl_get_trace_nodes` and `openl_export_trace` now wait server-side while the trace is still running instead of surfacing 409 Conflict to the caller (EPBDS-16089). While the backend answers 409, the tools retry internally every 2s (bounded by `waitTimeoutMs`, default 120s, cap 600s, with MCP progress notifications), so an LLM agent that cannot sleep between calls completes the start → read workflow in one call instead of burning its step budget polling. Pass `wait: false` for the previous immediate-409 behavior; on timeout the error explains the trace is still running and how to proceed.
