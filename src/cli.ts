...
function categoryOf(toolName: string): string {
  ...
  if (/^openl_(list_projects|get_project|open_project|save_project|close_project|create_project_branch|list_project_local_changes|restore_project_local_change|upload_file|download_file|start_project_tests|get_test_results)/.test(toolName)) {
    return "Project";
  }
  return "Other";
}
